...
function fixture(){
  const db=openDatabase(':memory:',{bootstrapPassword:'Test-only!123'}),admin=db.prepare('SELECT id FROM users WHERE email=?').get('admin@cj.net');
...
test('Gmail relay uses intermediary From, original Reply-To, immutable attachment and private errors',async()=>{
  ...
  try{
    ...
    queueMail(db,{key:'po:'+doc,order_id:order,kind:'SUPPLIER_PO',sender:'admin@cj.net',recipients:['vendor@example.com'],subject:'PO test',body:'<p>PO</p>',document_id:doc,status:'BLOCKED_CONFIG'});
    ...
    await service.flush();
    assert.equal(captured.from.address,DEFAULT_GMAIL);assert.equal(captured.replyTo.address,'admin@cj.net');assert.deepEqual(captured.to,[{address:'vendor@example.com'}]);assert.deepEqual(captured.attachments[0].content,bytes);assert.equal(captured.attachments[0].filename,'PO-PHÒNG BAN-18.09.2026-001.xlsx');assert.equal(captured.disableFileAccess,true);assert.equal(captured.disableUrlAccess,true);
    const event=db.prepare('SELECT * FROM outbox').get();assert.equal(event.status,'ACCEPTED');assert.equal(event.actual_sender,DEFAULT_GMAIL);assert.equal(event.reply_to,'admin@cj.net');assert.equal(event.provider,'GMAIL');assert.equal(db.prepare('SELECT status FROM orders').get().status,'SUBMITTED');
...
test('Gmail partial acceptance retries only recipients not accepted; never silently resends all',async()=>{
  ...
  try{
    const recipients=['manager@cj.net','admin@cj.net','extra@example.com'];
