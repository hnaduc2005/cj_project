...
export function saveCatalogItem(db,input,{forceNew=false}={}){
 ...
 let seq=existing?.sequence;if(!seq){seq=Number(db.prepare("SELECT value FROM settings WHERE key='catalog_sequence'").get()?.value||0)+1;db.prepare('INSERT INTO settings VALUES (?,?) ON CONFLICT(key) DO UPDATE SET value=excluded.value').run('catalog_sequence',String(seq));}
