...
export function nextOrderNumber(db, warehouse, timestamp = new Date(), departmentName = warehouse.name) {
  ...
  let number;
  do {
    const row = db.prepare('INSERT INTO order_sequences VALUES (?,?,1) ON CONFLICT(warehouse_id,local_date) DO UPDATE SET last_number=order_sequences.last_number+1 RETURNING last_number').get(warehouse.id, day.key);
