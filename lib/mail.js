...
export function queueMail(db, event) {
  const stamp = now();
  db.prepare('INSERT OR IGNORE INTO outbox(id,event_key,order_id,kind,sender,recipients,subject,body,document_id,status,error,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)').run(id(), event.key, event.order_id, event.kind, event.sender, JSON.stringify(event.recipients || []), event.subject, event.body, event.document_id || null, event.status || 'QUEUED', event.error || '', stamp, stamp);
}
